...
import { getInitials, goalLabel, statusLabel, formatDate } from "@/shared/lib/utils";
...
import { useClientPrograms } from "../hooks/useClientProgram";
...
export default function ClientDetail() {
  ...
  const { clientPrograms, loadClientPrograms } = useClientPrograms(Number(id));
  ...
  return (
    <div>
        ...
        <div className="flex items-center space-x-4">
          <div className="flex items-center space-x-4">
            ...
            <div className="flex flex-col">
              <h2 className="text-2xl font-semibold mt-2 inline">
                {client.name}
              </h2>
      ...
      <div className="mt-6 pt-8">
        <h2> Programas asignados </h2>
        {clientPrograms.length === 0 ? (
          <p className="text-muted-foreground text-sm text-center py-8">
            No se encontraron programas asignados al cliente
          </p>
        ) : (
            
          clientPrograms.map((clientProgram: ClientProgram, index: number) => (
            <div
              key={clientProgram.id}
              onClick={() => navigate(`/client-programs/${clientProgram.id}`)}
              className={`
                flex items-center gap-4 px-4 py-3 cursor-pointer
                hover:bg-accent transition-colors
                ${index !== clientPrograms.length - 1 ? "border-b" : ""}
              `}
            >
              <div
                className={`
    w-10 h-10 rounded-lg flex items-center justify-center text-lg flex-shrink-0
    ${clientProgram.program_type === "both" ? "bg-blue-500/10" : ""}
    ${clientProgram.program_type === "training" ? "bg-green-500/10" : ""}
    ${clientProgram.program_type === "nutrition" ? "bg-orange-500/10" : ""}
`}
              >
                {clientProgram.program_type === "both" && "💪"}
                {clientProgram.program_type === "training" && "🏋️"}
                {clientProgram.program_type === "nutrition" && "🥗"}
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm">
                  {clientProgram.program_name}
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">
                  Inicio: {formatDate(clientProgram.start_date)} ·{" "}
                  {clientProgram.duration_months} meses ·{" "}
                  {clientProgram.program_type}
                </p>
              </div>
              <Badge variant="outline" className="text-xs">
                {statusLabel(clientProgram.status)}
              </Badge>
